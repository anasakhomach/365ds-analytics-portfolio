...
- 2026-06-19: Fixed disappointing AI Helper behavior for assistant self/capability prompts. Regression tests were added for "how can you help me", "can you run or write sql queies", and LangGraph capability routing. Verification: focused assistant tests returned 15 passed; full hub tests returned 34 passed; `py_compile` passed for `streamlit_app.py` and `assistant.py`; `build_index.py --check` still reported 104 documents and 464 chunks. Docker rebuild was initially blocked by the approval system usage limit.
