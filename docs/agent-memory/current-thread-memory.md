...
- 2026-06-19: Learning Hub Batch 3 added provider/gateway/BYOK/Chroma runbooks, architecture docs, and ADR-001. Final verification: hub tests returned 23 passed; explicit `py_compile` passed; `build_index.py --check` and local build reported 104 documents and 464 chunks; Streamlit responded HTTP 200 on `http://localhost:8507`; Docker Compose config checks passed; Docker build remains blocked because Docker Desktop's Linux engine pipe `dockerDesktopLinuxEngine` is not running.
...
- Start Docker Desktop's Linux engine before rerunning `docker compose build learning-hub` and `docker compose run --rm indexer`.
